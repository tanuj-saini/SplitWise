...
io.on("connection", (socket) => {
    socket.on("Id", (data) => {
        ...
        // Check if the socket is already in the user's list
    ...
    socket.on("messageEvent", async (msg) => {
        const { groupId, userId, message } = msg;

        // Publish the message to the Redis channel named after the groupId
        pub.publish(groupId, JSON.stringify({ ...msg, createdBy: userId }));
...
sub.on("message", async (channel, message) => {
    ...
    if (groupClients) {
        // Forward the message to all users in the group, including the sender
        userList.forEach((userId) => {
            if (groupClients.has(userId)) {
                groupClients.get(userId).forEach((socket) => {
